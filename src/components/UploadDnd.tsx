...
function UploadDnd() {
  ...
  return preview ? (
    <div className="relative max-w-[500px] max-h-[300px] flex items-center justify-center">
      ...
      <Image className="object-cover rounded-md" src={preview} onLoad={() => setLoading(false)} width={400} height={200} alt="preview" />
      <button className="absolute top-4 right-4 z-10 backdrop-blur-md rounded-full" onClick={() => setPreview("")}>
        <svg
